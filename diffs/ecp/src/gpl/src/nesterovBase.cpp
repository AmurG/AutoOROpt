...
float Bin::overflow() const
{
  if (targetDensity_ <= 0.0f) {
    return 0.0f;
  }
  return std::max(0.0f, density_ - targetDensity_);
}
...
float BinGrid::averageOverflow(const odb::Rect& region) const
{
  if (bins_.empty()) {
    return 0.0f;
  }
  ...
  for (const auto& bin : bins_) {
    odb::Rect bin_rect(bin.lx(), bin.ly(), bin.ux(), bin.uy());
    if (!region.intersects(bin_rect)) {
      continue;
    }
    sum += static_cast<double>(bin.overflow());
    ++count;
  }
...
float BinGrid::maxOverflow(const odb::Rect& region) const
{
  ...
  for (const auto& bin : bins_) {
    ...
    max_overflow = std::max(max_overflow, bin.overflow());
  }
  return max_overflow;
}
