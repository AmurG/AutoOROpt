...
class BinGrid
{
 public:
  ...
  float averageOverflow(const odb::Rect& region) const;
  float maxOverflow(const odb::Rect& region) const;
...
class NesterovPlaceVars
{
 public:
  ...
  bool td_enable_dynamic_weights = true;
  float td_weight_min = 1.0f;
  float td_weight_max = 6.0f;
  float td_congestion_alpha = 0.7f;
  int td_ramp_iterations = 20;
  int td_update_period = 5;
  float td_initial_nets_percent = 10.0f;
  float td_final_nets_percent = 30.0f;
  int td_top_endpoints = 200;
  float td_slack_thresh = 0.0f;
  double td_slack_norm = 5e-11;
  double td_length_norm = 500.0;
  float td_overflow_limit = 0.4f;
  float td_severity_slack_norm = 0.2f;
  float td_severity_ratio_cap = 4.0f;
  float td_severity_weight_scale = 0.20f;
  float td_severity_weight_limit = 1.8f;
  float td_severity_coverage_scale = 0.5f;
  float td_severity_coverage_limit = 2.0f;
  float td_noncrit_slack_budget_ns = 0.05f;
