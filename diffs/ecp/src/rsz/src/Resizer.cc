...
VertexSeq Resizer::findWorstSlackVertices(int max_count, Slack slack_threshold)
{
  ...
  for (Vertex* vertex : *endpoints) {
    ...
    if (pin == nullptr || !network_->isLoad(pin)) {
      continue;
    }
    ...
    if (slack_threshold < slack) {
      continue;
    }
    if (heap.size() < static_cast<size_t>(max_count)) {
      heap.push({vertex, slack});
    } else if (slack < heap.top().slack) {
      heap.pop();
      heap.push({vertex, slack});
    }
  }
  ...
  while (!heap.empty()) {
    result.push_back(heap.top().vertex);
    heap.pop();
  }

  std::sort(result.begin(),
            result.end(),
            [this](Vertex* lhs, Vertex* rhs) {
              return sta_->vertexSlack(lhs, max_)
                     < sta_->vertexSlack(rhs, max_);
            });

  return result;
}

std::vector<odb::dbNet*> Resizer::criticalPathNets(int top_endpoints,
                                                   Slack slack_threshold)
{
  ...
  VertexSeq endpoints = findWorstSlackVertices(top_endpoints, slack_threshold);
  if (endpoints.empty()) {
    return nets;
  }

  std::unordered_set<odb::dbNet*> unique_nets;
  unique_nets.reserve(endpoints.size() * 4);

  for (Vertex* endpoint : endpoints) {
    const Path* path = sta_->vertexWorstSlackPath(endpoint, max_);
    if (path == nullptr) {
      continue;
    }
    PathExpanded expanded(path, sta_);
    const size_t start_index = expanded.startIndex();
    for (size_t idx = start_index + 1; idx < expanded.size(); ++idx) {
      const Path* step = expanded.path(idx);
      const Pin* pin = step->pin(sta_);
      if (pin == nullptr) {
        continue;
      }
      ...
      if (db_network_->isSpecial(sta_net) || sta_->isClock(pin)) {
        continue;
      }
      ...
      unique_nets.insert(db_net);
    }
  }

  nets.reserve(unique_nets.size());
  for (odb::dbNet* db_net : unique_nets) {
