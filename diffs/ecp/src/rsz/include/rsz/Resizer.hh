...
class Resizer : public dbStaState, public dbNetworkObserver
{
 public:
  ...
  std::vector<odb::dbNet*> criticalPathNets(int top_endpoints,
                                            Slack slack_threshold);
  VertexSeq findWorstSlackVertices(int max_count, Slack slack_threshold);
  odb::dbNet* staToDb(const sta::Net* net) const
  {
    return db_network_->staToDb(net);
  }
  double setupSlackGuard() const { return setup_slack_guard_; }
  void setSetupSlackGuard(double guard) { setup_slack_guard_ = guard; }
  double nonCritSlackBudgetNs() const { return non_critical_slack_budget_ns_; }
  void setNonCriticalSlackBudget(double budget)
  {
    non_critical_slack_budget_ns_ = std::max(0.0, budget);
  }
 ...
 protected:
  ...
  double setup_slack_guard_ = 5e-11;
